    ...
    if (OnAirDate == WorkData[0][0][0]) {
      ...
      if (w2 == WorkData[0][0][2]) {
        ...
        const ImageUrlText = await RPA.WebBrowser.driver
          .findElement(
            By.css(
              '#main > div > div.c-application-DesktopAppContainer__content-container > div.c-application-DesktopAppContainer__content > main > div > div > div > div.c-tv-UpcomingSlotContainerView__header > div.c-tv-UpcomingSlotContainerView__thumbnail-column > div > div > picture > source'
            )
          )
          .getAttribute('srcset');
      ...
      // 今夜
      const T24_00 = new Date(`${d}-${mS}-${y2} 24:00:00 GMT`);
      const T26_59 = new Date(`${d}-${mS}-${y2} 02:59:00 GMT`);
      // コピーライトを取得
      const CopyRight = await RPA.WebBrowser.findElementByClassName(
        'com-o-Footer__copyright'
      );
...
async function GoogleUpload(Renamming) {
  ...
  await RPA.Logger.info('アップロード完了');
}
